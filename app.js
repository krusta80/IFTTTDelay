...
app.use("//delay", function(req, res) {
    ...
    if (delay == undefined || key == undefined || event == undefined) {
        res.send(
            "Error: must specify t (for delay time) and key and event in the URL."
        );
        return;
    }
    ...
    var timerId = setTimeout(function() {
        ...
        console.log("URL = " + url);
        request.post(url, { form: bodyToSend }, function(
            error,
            response,
            body
        ) {});
...
app.use("/", function(req, res) {
    res.status(200).json({
        status: "ok",
        msg: "You've reached the API root. Service is running."
    });
});
